...
function ValidateEmployee() {
 ...
 const validateEmployeeNumber = (number) => {
  ...
  const validateRegistrationCode = (code) => {
    return /^[A-Za-z0-9-]{8,50}$/.test(code); // Keep existing registration code validation
  };
  ...
  const validateForm = () => {
    console.log('Validating:', formData.employeeNumber);
    const newErrors = {};
...
// Add this condition in your handleSubmit function
const handleSubmit = async (e) => {
  ...
  const handleProceedToRegistration = () => {
    if (userData) {
      navigate('/HCPRegister', { state: { userData } });
    }
  };
