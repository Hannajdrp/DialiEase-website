...
const HCProviderList = () => {
  ...
  const [providersPerPage] = useState(4); // Changed to show only 4 per page
  ...
  const generatePDF = () => {
    try {
      ...
      const headers = ['Employee No.', 'Name', 'Specialization'];
      const columns = [
        provider => provider.employeeNumber || 'N/A',
        provider => `${provider.first_name} ${provider.middle_name || ''} ${provider.last_name}`.trim() || 'N/A',
  ...
  const handleBatchCall = () => {
    alert('Batch call functionality would be implemented here');
  };
  ...
  const styles = {
    ...
    content: {
      ...
      maxWidth: sidebarOpen && !isMobile ? 'calc(100vw - 250px)' : '100vw',
    },
    mobileHeader: {
      display: 'flex',
  ...
  return (
    <div style={styles.container}>
      <StaffSidebar isOpen={sidebarOpen} toggleSidebar={toggleSidebar} isMobile={isMobile} />
        ...
        <div style={styles.header}>
          ...
          <div style={styles.headerControls}>
            <div style={styles.searchBox} ref={searchRef}>
              ...
              <input
                type="text"
                placeholder="Search by name, employee number, specialization..."
