...
import { 
  ...
  FaBell, FaSearch, FaSort, FaCalendarAlt, FaCalendar
} from 'react-icons/fa';
import {
  Table,
  TableBody,
...
import Resched_ReasonList from './Resched_ReasonList';
...
const PatientScheduleList = () => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      setMissedSchedules(missedResponse.data.data);
  ...
  const sortedMissedSchedules = React.useMemo(() => {
    let sortableItems = [...missedSchedules];
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        if (a[sortConfig.key] < b[sortConfig.key]) {
          return sortConfig.direction === 'asc' ? -1 : 1;
  ...
  const generatePDF = () => {
    try {
      ...
      const tabTitles = ['All Schedules', 'Upcoming', 'Missed', 'Completed'];
      doc.text(`Report Type: ${tabTitles[value]}`, 14, 46);

      const headers = ['Patient Name', 'Hospital No.', 'Appointment Date', 'Status', 'Remarks'];
      const columns = [
        schedule => `${schedule.patient?.first_name || ''} ${schedule.patient?.last_name || ''}`.trim() || 'N/A',
        schedule => schedule.patient?.hospitalNumber || 'N/A',
        schedule => format(parseISO(schedule.appointment_date), 'MMM dd, yyyy'),
        schedule => schedule.checkup_status || 'N/A',
        schedule => schedule.remarks || 'N/A'
      ];
  ...
  return (
    <div style={styles.container}>
        ...
        <div style={styles.header}>
          ...
          <div style={styles.headerControls}>
            ...
            <div style={styles.actionButtons}>
              ...
              >
                <FaCalendar style={styles.buttonIcon} />
                {isMobile ? '' : 'Reschedule Requests'}
              </button>
              <button 
      ...
      {showRescheduleModal && (
        <Resched_ReasonList onClose={() => setShowRescheduleModal(false)} />
      )}
